...
namespace CSSL.Modeling
{
    public class Executive
    {
        ...
        internal void ScheduleEvent(double time, CSSLEventAction action, int id)
        {
            if (time < Time)
            {
                throw new Exception("Attempted to schedule an event in the past.");
            }
            CSSLEvent e = new CSSLEvent(time, action, id);
            calendar.Add(e);
        }

        internal void ScheduleEvent(double time, CSSLEventAction action, int id, int modelElementId)
        {
            if (time < Time)
            {
                throw new Exception("Attempted to schedule an event in the past.");
            }
            CSSLEvent e = new CSSLEvent(time, action, id, modelElementId);
            calendar.Add(e);
        }

        internal void ScheduleEvent(double time, CSSLEventAction action, int id, int modelElementId, int subModelElementId)
        {
            if (time < Time)
            {
                throw new Exception("Attempted to schedule an event in the past.");
            }
            CSSLEvent e = new CSSLEvent(time, action, id, modelElementId, subModelElementId);
            calendar.Add(e);
        }
        ...
        internal void ScheduleEndEvent(double time, CSSLEventAction action, int id)
        {
            if (time < Time)
            {
                throw new Exception("Attempted to schedule an event in the past.");
        ...
        private class EventExecutionProcess : IterativeProcess<CSSLEvent>
        {
            ...
            private void ScheduleEndEvent()
            {
                ...
                executive.ScheduleEndEvent(executive.MySimulation.MyExperiment.LengthOfReplication, executive.HandleEndEvent, 12345);
            }
